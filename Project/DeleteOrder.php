...
<body>
    ...
    <div class=m>
        <div class="myPadding">
            <center>
                <h2 class=title><b>CANCEL ORDER</b></h2>
            </center>
            <fieldset>
                <legend align=center>Enter the order number of the order you would like to cancel</legend>
                <?php
  include("config.php");
   session_start();
  ...
  if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql="select * FROM itemdetail JOIN dropofdetail ON (itemdetail.itemdetailID = dropofdetail.dropofdetailID) JOIN payment ON (itemdetail.itemdetailID=payment.paymentId) JOIN pickupdetail ON (itemdetail.itemdetailID=pickupdetail.pickupdetailID) where itemdetailID = '$num'";
      ...
      if($count == 1) {            
            $_SESSION['Number'] = $num;
          $update= "UPDATE `payment` SET `state`='Cancled by user' WHERE `paymentId`='$num'";
          $update_result = mysqli_query($connection,$update);
          if($update_result){
              echo "Would you like to oder another order?";
          }else {
              echo "Oops! Something went wrong.";
          }
          $message = "Sucesfully Canceled";
...
?>
                    <form class="form-signin" method="POST">
                        ...
                        <div class="row">
                            ...
                            <div class="column2">
                                <center> <input type="submit" value="Cancel" id=delete name=delete>
                                </center>
                            </div>
                        </div>
                    </form>
